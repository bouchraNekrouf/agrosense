...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Envoi de la demande de connexion (Sign in)
    ...
    if (signForm) {
        signForm.addEventListener('submit', async (e) => {
            ...
            const requireds = signForm.querySelectorAll('[required]');
            let formValid = true;
            ...
            // Afficher l'état de chargement pour le bouton
            ...
            try {
                // Envoyer les données à notre API (Node.js)
                ...
                if (response.ok) {
                    ...
                    if (successMsg) {
                        successMsg.querySelector('h3').innerText = "Connexion réussie!";
                        successMsg.querySelector('p').innerText = "Bienvenue de nouveau " + result.userName;
                        ...
                        alert("Bienvenue de nouveau " + result.userName + "!\n" + result.message);
                    }

                    // Sauvegarder les données dans le navigateur pour rester connecté et les afficher dans le profil
                    ...
                    // Rediriger l'utilisateur vers la page d'accueil (index.html) ou le lien demandé
                    const urlParams = new URLSearchParams(window.location.search);
let redirectPage = urlParams.get('redirect');
if (!redirectPage) {
    if (result.role === 'admin') {
        redirectPage = "../apres-inscription/admin/index.html";
    } else {
        redirectPage = "../index.html";
                ...
                // FALLBACK HORS LIGNE: Fonctionne même si le serveur Node.js est éteint
                ...
                alert("Mode local (Hors Ligne): Connexion réussie ! " + data.email.split('@')[0]);
                setTimeout(() => {
                    const urlParams = new URLSearchParams(window.location.search);
