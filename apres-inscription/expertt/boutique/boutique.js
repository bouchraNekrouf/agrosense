...
function initCategories() {
    ...
    // Fill Sidebar with translated labels
    const list = document.getElementById('category-list');
    if (!list) return;
    const allLabel = dict['Toutes les catégories'] || 'Toutes les catégories';
    list.innerHTML = `<li class="active" onclick="filterByCategory('Toutes les catégories', this)">${allLabel}</li>`;
    categories.forEach(cat => {
        const escapedCat = cat.replace(/'/g, "\\'");
        const translatedCat = dict[cat] || cat;
        list.innerHTML += `<li onclick="filterByCategory('${escapedCat}', this)">${translatedCat}</li>`;
    });
}
...
function filterByCategory(catName, element) {
    ...
    const displayName = dict[catName] || catName;
    document.getElementById('current-category-title').textContent = displayName;
...
function renderFellahProducts() {
    ...
    if (filtered.length === 0) {
        const lang2 = localStorage.getItem('site_lang') || 'fr';
        const dict2 = (typeof translations !== 'undefined' && translations[lang2]) ? translations[lang2] : {};
        const msg = dict2['Aucun produit disponible dans cette catégorie pour le moment.'] || 'Aucun produit disponible dans cette catégorie pour le moment.';
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align:center; color: var(--text-muted); padding: 3rem;">${msg}</p>`;
        return;
    }

    // Get current language dict once
    const lang = localStorage.getItem('site_lang') || 'fr';
    const dict = (typeof translations !== 'undefined' && translations[lang]) ? translations[lang] : {};
    ...
    // 4 - Translate "Ajouter au panier"
    const addToCartLabel = dict['Ajouter au panier'] || 'Ajouter au panier';
        ...
        if (isExpertPreview) {
            ...
            <div style="display: flex; flex-direction: column; gap: 8px;">
                <div style="display: flex; align-items: center; justify-content: space-between; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; background: white;">
                    ...
                    <input type="number" id="qty-${prodId}" value="1" min="1" style="width: 100%; text-align: center; border: none; font-size: 1rem; font-weight: 600; color: #000000; outline: none; -moz-appearance: textfield;" readonly>
                ...
                <button style="width: 100%; background: #86efac; border: none; color: #000000; padding: 10px; border-radius: 8px; font-size: 0.95rem; font-weight: 700; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: background 0.2s;" onmouseover="this.style.background='#4ade80'" onmouseout="this.style.background='#86efac'" onclick="addToCart('${prodId}')"><i class="fas fa-cart-plus"></i> ${addToCartLabel}</button>
            </div>`;
        }

        // 1 - Translate category label, 2 - price color white
        const translatedCat = dict[prod.category] || prod.category;
        card.innerHTML =
            '<img src="' + (prod.image || DEFAULT_IMAGE) + '" alt="' + prod.name + '" class="product-img">' +
            '<div class="product-info">' +
            '<span class="product-cat">' + translatedCat + '</span>' +
            '<h4 class="product-name">' + prod.name + '</h4>' +
            '<div class="product-price" style="color: #ffffff;">' + parseFloat(prod.price).toFixed(2) + ' DA</div>' +
            actionHtml +
            '</div>';
